...
static void runpage(fz_document *doc, int number, fz_document_writer *out) {
    ...
    fz_page *page;
    fz_device *dev = NULL;
    fz_try(ctx) page = fz_load_page(ctx, doc, number - 1); fz_catch(ctx) ereport(ERROR, (errmsg("fz_load_page: %s", fz_caught_message(ctx))));
    fz_try(ctx) (void)fz_var(dev); fz_catch(ctx) ereport(ERROR, (errmsg("fz_var: %s", fz_caught_message(ctx))));
    fz_try(ctx) mediabox = fz_bound_page(ctx, page); fz_catch(ctx) ereport(ERROR, (errmsg("fz_bound_page: %s", fz_caught_message(ctx))));
    fz_try(ctx) dev = fz_begin_page(ctx, out, mediabox); fz_catch(ctx) ereport(ERROR, (errmsg("fz_begin_page: %s", fz_caught_message(ctx))));
    fz_try(ctx) (void)fz_run_page(ctx, page, dev, fz_identity, NULL); fz_catch(ctx) ereport(ERROR, (errmsg("fz_run_page: %s", fz_caught_message(ctx))));
    fz_try(ctx) (void)fz_end_page(ctx, out); fz_catch(ctx) ereport(ERROR, (errmsg("fz_end_page: %s", fz_caught_message(ctx))));
    fz_try(ctx) (void)fz_drop_page(ctx, page); fz_catch(ctx) ereport(ERROR, (errmsg("fz_drop_page: %s", fz_caught_message(ctx))));
}
...
EXTENSION(pg_mupdf) {
    ...
    fz_try(ctx) buf = fz_new_buffer(ctx, VARSIZE_ANY_EXHDR(data)); fz_catch(ctx) ereport(ERROR, (errmsg("fz_new_buffer: %s", fz_caught_message(ctx))));
    fz_try(ctx) (void)fz_append_data(ctx, buf, VARDATA_ANY(data), VARSIZE_ANY_EXHDR(data)); fz_catch(ctx) ereport(ERROR, (errmsg("fz_append_data: %s", fz_caught_message(ctx))));
    fz_try(ctx) stream = fz_open_buffer(ctx, buf); fz_catch(ctx) ereport(ERROR, (errmsg("fz_open_buffer: %s", fz_caught_message(ctx))));
    fz_try(ctx) doc = fz_open_document_with_stream(ctx, input, stream); fz_catch(ctx) ereport(ERROR, (errmsg("fz_open_document_with_stream: %s", fz_caught_message(ctx))));
    fz_try(ctx) out = fz_new_document_writer(ctx, NULL, output, options); fz_catch(ctx) ereport(ERROR, (errmsg("fz_new_document_writer: %s", fz_caught_message(ctx))));
